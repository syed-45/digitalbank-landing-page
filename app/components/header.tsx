...
export const Header = ({isMenuOpen, setMenuIsOpen}: IHeaderProps): JSX.Element => {
    return (
        <header className="flex justify-center pl-5 pr-7 py-6 bg-white">
            <div className="w-full grow">
                <Image src="/logo-dark.svg" alt="logo" width={5000} height={5000} className="w-[150px]"/>
            </div>
            <button 
                className={`transition transition-discrete starting:opacity-0 opacity-100 duration-400`}
            ...
            <div 
                className={`md:hidden transition transition-discrete duration-500 ${isMenuOpen ? "block opacity-100 " : "hidden opacity-0 "} starting:opacity-0 h-dvh w-full bg-linear-to-b from-black/50 absolute top-[69.4px] left-0 flex`}
